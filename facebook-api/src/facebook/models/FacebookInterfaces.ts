...
export interface AdsLibrarySearchParams {
  ...
  ad_reached_countries: CountryCode[]; // Ce paramètre est requis.
  ad_type?:
    | 'ALL'
    | 'CREDIT_ADS'
    | 'EMPLOYMENT_ADS'
...
export interface ProcessedAds {
  ads: {
    data: TransformedAd[]; // Modifier la propriété en "data" au lieu de "ads"
...
export interface AgeCountryGenderReachBreakdown {
  // Structure selon la description
}
...
export interface GPTads {
  ad_creative_bodies: string;
  age_country_gender_reach_breakdown: any[]; // Consider specifying further if possible
  eu_total_reach: number;
  publisher_platforms: string[];
  target_ages: string[];
  target_gender: string;
  target_locations: any[]; // Consider specifying further if possible
  creative_src: string;
  full_ad_src: string;
  landing_page_src: string;
  ad_delivery_start_time: string;
...
export interface AdResponse {
  ...
  age_country_gender_reach_breakdown: any; // AgeCountryGenderReachBreakdown[];
  beneficiary_payers: any; // BeneficiaryPayer[];
  bylines: string;
  currency: string;
  delivery_by_region: any; // AudienceDistribution[];
  demographic_distribution: any; // AudienceDistribution[];
  estimated_audience_size: any; // InsightsRangeValue;
  eu_total_reach: number; // int32
  impressions: any; // InsightsRangeValue;
  languages: string[];
  page_id: string; // numeric string
  page_name: string;
  publisher_platforms: string[]; // list<enum> peut nécessiter une définition plus précise en fonction de l'utilisation
  spend: any; // InsightsRangeValue;
  target_ages: string[]; // list<numeric string>
  target_gender: 'Women' | 'Men' | 'All';
  target_locations: any; // TargetLocation[];
}
...
export interface Choice {
  ...
  logprobs: null; // Assuming logprobs is always null based on the provided structure. Adjust accordingly if it varies.
...
export interface MessagePayload {
  role: string;
  content: Content[]; // Now content is an array of Content
}
